...
def focus_zone(im, fill=True):
    ...
    # Fill small areas
    rough_subject = remove_blobs(blurred, area=1000, connectivity=1)

    if fill:
        cnts = cv2.findContours(rough_subject.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
for path in paths:
    ...
    # Extract the focus area from L and S channels independently, and only take whatever is common to both
    focus_area_L = focus_zone(L)
    focus_area_S = focus_zone(S)
    focus_area_both = focus_area_S.astype(bool) & focus_area_L.astype(bool)
    focus_area = cv2.GaussianBlur((focus_area_both * 255).astype(np.uint8), (17, 17), 0)
